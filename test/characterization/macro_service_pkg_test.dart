...
void main() {
  group(
    ...
    test('user macros: {{user}}/{{persona}} -> userName', () {
      expect(macro.process('Hi {{user}} and {{persona}}'), 'Hi Alice and Alice');
    });
    ...
    group('conditional {{if::...}} macro', () {
      ...
      test('falsey condition with 4 args yields empty (else is unreachable)',
          () {
  ...
  group('SUT=package | MacroService nondeterministic macros (invariants only)',
      () {
