// Package-internal model: regex script (rule definition + editor state).
//
// Provenance: verbatim copy of
//   lib/features/prompt_lab/data/models/regex_script.dart
...
class RegexScript {
  ...
  factory RegexScript.fromJson(Map<String, dynamic> json) {
    return RegexScript(
      ...
      trimStrings:
          (json['trimStrings'] as List<dynamic>?)
              ?.map((e) => e as String)
              .toList() ??
          [],
      placement:
          (json['placement'] as List<dynamic>?)
              ?.map(
                (e) => RegexPlacement.values.firstWhere(
                  (p) => p.name == e,
